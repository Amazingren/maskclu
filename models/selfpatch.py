...
class ClassAttention(nn.Module):
    ...
    def forward(self, x, attention=False, mask=None):
        ...
        if mask is not None:
            mask_temp = torch.cat([torch.ones(B, 1).bool().cuda(), mask], dim=1).unsqueeze(1).unsqueeze(1).expand(
...
class LayerBlockCa(nn.Module):
    # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
    # with slight modifications to add CA and LayerScale
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, Attention_block=ClassAttention,
                 Mlp_block=Mlp):
        super().__init__()
    ...
    def __init__(self, in_dim, num_heads):
        ...
        self.cls_blocks = nn.ModuleList([
            LayerBlockCa(
                ...
                act_layer=nn.GELU, Attention_block=ClassAttention,
                Mlp_block=Mlp)
            for i in range(2)])
...
class DINOLoss(nn.Module):
    ...
    @torch.no_grad()
    def update_patch_center(self, batch_center):
