...
class MaskTransformer(nn.Module):
    ...
    def _mask_center_block(self, center, noaug=False, is_new=False):
        '''
            center : B G 3
            --------------
            mask : B G (bool)
    ...
    def _mask_center_rand(self, center, noaug=False, is_new=False):
        '''
            center : B G 3
            --------------
            mask : B G (bool)
    ...
    def forward(self, neighborhood, center, noaug=False, is_new=False, is_eval=False):
        # generate mask
        if self.mask_type == 'rand':
            bool_masked_pos = self._mask_center_rand(center, noaug=noaug, is_new=is_new)  # B G
        else:
            bool_masked_pos = self._mask_center_block(center, noaug=noaug, is_new=is_new)
        ...
        if is_eval:
            ...
            x = self.blocks(torch.cat([cls_tokens, group_input_tokens], dim=1), torch.cat([cls_pos, pos], dim=1))
