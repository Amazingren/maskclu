...
from libs.lib_utils import random_scale_point_cloud, shift_point_cloud
...
def parse_args():
    ...
    parser.add_argument('--root', type=str, default='/data/disk1/data',
                        help='data root')
    parser.add_argument('--class_choice', type=str, default=None, metavar='N', choices=[
        'airplane', 'bag', 'cap', 'car', 'chair', 'earphone', 'guitar', 'knife', 'lamp', 'laptop', 'motor', 'mug',
...
def main(args):
    ...
    train_dataset = ShapeNetPart(root, partition='trainval', num_points=args.npoint, class_choice=args.class_choice)
    ...
    test_dataset = ShapeNetPart(root, partition='test', num_points=args.npoint, class_choice=args.class_choice)
    ...
    classifier = SegTransformer(num_part).cuda()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        num_iter = 0
        '''learning one epoch'''
        for i, (points, label, target) in tqdm(enumerate(train_data_loader), total=len(train_data_loader),
                                               smoothing=0.9):
            num_iter += 1
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target) in tqdm(enumerate(test_data_loader), total=len(test_data_loader),
                                                          smoothing=0.9):
