...
from libs.AverageMeter import AverageMeter
from models.maskclu import MaskClu
from tools import builder, dist_utils, misc
from tools.logger import get_logger, print_log

train_transforms = transforms.Compose(
    [
        # data_transforms.PointcloudScale(),
        transform.PointcloudRotate(),
        # data_transforms.PointcloudTranslate(),
        transform.PointcloudJitter(),
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # build model
    base_model = MaskClu(config.model)
    if args.use_gpu:
    ...
    if args.resume:
        ...
        best_metrics = AccMetric(best_metric)
    ...
    if args.distributed:
        ...
        base_model = nn.parallel.DistributedDataParallel(
            base_model, device_ids=[args.local_rank % torch.cuda.device_count()], find_unused_parameters=True)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        ass_losses = AverageMeter(['Ass_loss'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            g_loss, ass_loss = base_model(points)
            loss = g_loss + ass_loss
            try:
                loss.backward()
                # print("Using one GPU")
            except:
            ...
            if args.distributed:
                ...
                ass_loss = dist_utils.reduce_tensor(ass_loss, args)
            ...
            if idx % 20 == 0:
                print_log('[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) '
                          'Losses = %s  assign losses = %s lr = %.6f' %
                          (epoch, config.max_epoch, idx + 1, n_batches, batch_time.val()[0], data_time.val()[0],
                           ['%.4f' % l for l in losses.val()], ['%.4f' % t for t in ass_losses.val()],
