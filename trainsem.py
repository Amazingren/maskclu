...
from models.semseg import SegTransformer
...
def parse_args():
    ...
    parser.add_argument('--optimizer_part', type=str, default='all',
                        help='training all parameters or optimizing the new layers only')
...
def main(args):
    ...
    '''MODEL LOADING'''
    classifier = SegTransformer(num_classes).cuda()
    ...
    scheduler = CosineLRScheduler(optimizer,
                                  ...
                                  cycle_mul=1.,  # FIXME unexpected parameter
                                  lr_min=1e-6,
                                  cycle_decay=0.1,
                                  warmup_lr_init=1e-6,
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            loss = classifier.get_loss(seg_pred, target, weights)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            mIoU = np.mean(np.array(total_correct_class) / (
                    np.array(total_iou_deno_class, dtype=float) + 1e-6))  # np.float -> float
            ...
            log_string('[mAcc] eval point avg class acc: %f' % (
                    np.mean(np.array(total_correct_class) / (np.array(total_seen_class, dtype=float) + 1e-6)) * 100.0))
